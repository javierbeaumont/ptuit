...
function validaForm()
{
	...
	var rpass=eliminaEspacios(form.inputRpass.value);
	var pass=eliminaEspacios(form.inputPass.value);
	var correo=eliminaEspacios(form.inputCorreo.value);
	
	
	if(!validaLongitud(user, 0, 4, 50)) campoError(form.inputUser);
	if(!validaLongitud(rpass, 0, 8, 50)) campoError(form.inputRpass);
	if(!validaLongitud(pass, 0, 8, 50)) campoError(form.inputPass);
	...
	else
	{
		...
		ajax.send("user="+user+"&pass="+pass+"&rpass="+rpass+"&correo="+correo);
